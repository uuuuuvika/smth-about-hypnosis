...
static int usage(const char *progname, const char *msg = NULL) {
  ...
  fprintf(stderr, "usage: %s [options] <video-or-directory> [<video-or-directory>...]\n", progname);
  ...
  fprintf(stderr, "\nNotes:\n");
  fprintf(stderr, "  - You can pass directories. All video files in the directory are enqueued sorted by name.\n");
  fprintf(stderr, "  - Supported extensions include: .mp4, .mov, .m4v, .avi, .mkv, .webm\n");
  return 1;
}
...
static bool HasVideoExtension(const std::string &filename) {
...
static std::vector<std::string> ExpandInputsToFiles(int argc, char *argv[], int start_index, bool verbose) {
  std::vector<std::string> files;
  for (int i = start_index; i < argc; ++i) {
    const char *arg = argv[i];
    if (strcmp(arg, "-") == 0) {
      files.emplace_back("/dev/stdin");
      continue;
    }
    if (IsDirectory(arg)) {
      DIR *dir = opendir(arg);
      if (!dir) {
        if (verbose) perror("Could not open directory");
        continue;
      }
      ...
      while ((entry = readdir(dir)) != NULL) {
        ...
        std::string fullpath = std::string(arg) + "/" + entry->d_name;
        if (IsRegularFile(fullpath.c_str()) && HasVideoExtension(fullpath)) {
          dir_files.push_back(fullpath);
        }
      }
      closedir(dir);
      std::sort(dir_files.begin(), dir_files.end());
      if (dir_files.empty() && verbose) {
        fprintf(stderr, "No video files found in directory: %s\n", arg);
      }
...
int main(int argc, char *argv[]) {
  ...
  if (optind >= argc) {
    fprintf(stderr, "Expected video file or directory.\n");
    return usage(argv[0]);
  }

  // Expand any directory arguments into lists of files (sorted)
  std::vector<std::string> movie_files = ExpandInputsToFiles(argc, argv, optind, verbose);
  if (movie_files.empty()) {
    fprintf(stderr, "No playable input files found.\n");
    return usage(argv[0]);
  }
  const bool multiple_videos = (movie_files.size() > 1);
  ...
  do {
    for (size_t m = 0; m < movie_files.size() && !interrupt_received; ++m) {
      const char *movie_file = movie_files[m].c_str();
